...
export default function PercentileRanks({
  ...
  return (
    <div>
      <SectionHeader
        title="Rank among filers"
        caption={`SEC XBRL frames, ${period}`}
      />
      ...
      <p className="mt-3 font-sans text-2xs leading-relaxed text-muted">
        Ranked against every filer that tagged the same concept for {period},
        not against all public companies. Frames group filings by approximate
        calendar alignment rather than by fiscal year, so a company whose year
        ends part way through the next one is still counted here.
      </p>
    </div>
  );
}
